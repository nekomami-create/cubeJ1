...
分からなければ `find-cube.ps1` が LAN を舐めて探します。
...
ブラウザのメニューから「ホーム画面に追加」。緑の稲妻アイコンが置かれ、
タップでダッシュボードが直接開きます。URL を覚えておく必要はありません。

アイコンとマニフェストは `meter_hub.py` に埋め込んであり、Cube 自身が配信します。
`start_url` は root 相対なので、**Cube の IP をどこにも書かずに済みます**。

```
/manifest.webmanifest   ウェブアプリマニフェスト
/icon-192.png           Android 用（any と maskable を兼ねる）
/icon-512.png           同上、高解像度
/apple-touch-icon.png   iOS 用 180x180
```
...
| 端末 | 結果 |
|---|---|
| iPhone / Safari | アイコンから全画面で開く。アドレスバーなし |
| Android / Chrome | アイコンは出るが、**アドレスバーは残る** |

Android で全画面にならないのは、Chrome が「アプリとしてインストール」を
**HTTPS のページにしか許さない**ためです。Cube は平文 HTTP なので条件を
満たしません。家庭内 LAN 専用という前提から HTTPS にする意味も薄く、
自己署名証明書を入れると警告が出てかえって使いにくくなります。

Android で本当に全画面にしたい場合は、WebView を1枚かぶせた APK を作るのが
確実です。ロードマップの後半に置いてあります。

### IP が変わるとアイコンが切れる

ホーム画面のアイコンは追加した時点の URL を覚えます。Cube の IP が
DHCP で変わるとリンク切れになるので、**ルーターで IP を固定してから**
追加してください。
...
そのまま叩けます。レスポンスの形と実装上の注意は `docs/api.md` に
まとめてあります。
...
- init RC を書き終えた後に `mount -o ro,remount /` でルートを読み取り専用に戻す
  （上流は rw のまま放置している）
- 純正の `wisund.rc` / `ndeclite_agent.rc` を上書きする前に `/data/local/` へ
  退避する（上流は退避せず上書きする。純正の中身は他のどこにも残らない）
- `mqtt_bridge.py` の LED 出力先を `/dev/null` に向けてから配置する
  （USB 上のファイルは上流とバイト同一のまま。配置する側だけを書き換える）
