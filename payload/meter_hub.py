...
#
# Embedded as base64 rather than shipped as files: production_tool only ever
# copies meter_hub.py to /data/local, so a directory of assets would be one
# more thing to keep in step. Decoded once at import, not per request.
# binascii is used instead of the base64 module because the bridge already
# imports it, which proves it exists on this device.
#
# The mark is the same green bolt as FAVICON below, so the browser tab and
# the home screen show one icon. It predates the light dashboard palette and
# no longer matches it; restyling both together is a separate decision.
# ---------------------------------------------------------------------------
...
# start_url, scope and the icon paths are root-relative on purpose: they
# resolve against whatever address the phone used to reach the Cube, so the
# device's IP never has to appear here. background_color follows the page's
# declared default (light); a manifest cannot carry a media query.
MANIFEST = """{
  "name": "電力モニタ",
  "short_name": "電力モニタ",
